...
joined_points = gpd.sjoin(points, polygons[["Code_12", "geometry"]], how="left", predicate="within")
joined_points = joined_points.drop(columns="geometry")
points["Code_12_point"] = joined_points["Code_12"].values
...
intersections_of_category = intersections.groupby(["RELI", "Code_12"])["percent_per_square"].sum().reset_index()
...
intersections_of_category_from_square = intersections_of_category.groupby("RELI")[["Code_12", "percent_per_square"]].apply(lambda df: {row["Code_12"]: row["percent_per_square"] for _, row in df.iterrows()}).rename("corine_categories").reset_index()
...
squares_with_categories["IPCC_CORINE_Id"] = squares_with_categories["Code_12_point"].astype(int).map(ipcc_corine_mapping_dict)
...
squares_with_categories.to_file(DATA_DIR / "analysis/corine/2012/test1.gpkg")
